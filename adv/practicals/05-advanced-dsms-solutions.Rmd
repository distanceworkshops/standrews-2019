...
date: Solution 5. Advanced density surface models
output: 
  pdf_document:
...
Here is a "solution" for exercise 5. As with any data analysis, there is no correct answer, but this shows how I would approach this analysis. The analysis here is conditional on selecting a detection function in the previous exercises; I've chosen `df_hn` and `df_hr_ss_size` that we saved previously (see detection function practical).
...
n <- ddply(join_dat, .(Sample.Label), summarise, n=sum(size, na.rm=TRUE), .drop = FALSE) 
...
Finally, we can plot histograms of counts aggregated for different values of the covariates:
```{r}
p <- ggplot(segs_eda) +
...
```
Here we used the value of the truncation stored in the detection function object (`df_hn$ddf`), but we could also use the numeric value (which we can also find by checking the model's `summary()`).
...
2. We've not specified basis complexity (`k`) at the moment. Note that if you want to specify the same complexity for multiple terms, it's often easier to create a variable that can then be provided to `k` (for example, specify `k1 <- 15` and then set `k=k1` in the required `s()` terms).

<div class="boxout">
Looking at the `summary()` here we can see that many of the terms have been shrunk out of the model. We could simply use this model to make predictions at this point since those shrunken terms will have little effect on the results (since their EDFs are so small), but it makes sense to remove them (because predictions will be faster and we might avoid building many prediction grids for those covariates -- more on that later).
</div>
...
3. `scale=0` puts each plot on a different $y$-axis scale, making it easier to see the effects. Setting `scale=-1` will put the plots on a common $y$-axis scale so you can see the relative influence of the effects, as shown below.

```{r plot-nb-xy-scale}
plot(dsm_nb_xy_ms, pages=1, scale=-1)
```
...
Compare this plot to Figure 1 generated in the previous practical when only `x` and `y` were included in the model.
...
vis.gam(dsm_nb_xy, view=c("x","y"), plot.type="contour", too.far=0.1, 
        main="s(x,y) (spatial only, link scale)", asp=1)
vis.gam(dsm_nb_xy_ms, view=c("x","y"), plot.type="contour", too.far=0.1, 
        main="s(x,y) (full model, link scale)", asp=1)
```
...
Decide on a significance level that you'll use to discard terms in the model. Remove the terms that are non-significant at this level and re-run the above checks, summaries and plots to see what happens. It's helpful to make notes for yourself as you go. 
...
Having removed a smooth and reviewed your model, you may decide you wish to remove another. Repeat the process of removing a term and looking at plots and diagnostics again.
...
summary(dsm_tw_xy_ms)
```
Removing `SST` has moved us back to roughly the smooth terms EDF/$p$-values we had previously. Let's remove `NPP`...
```{r tw-xy-ms3}
dsm_tw_xy_ms <- dsm(count~s(x,y, bs="ts") +
                          s(Depth, bs="ts") +
                          s(EKE, bs="ts"),
...
summary(dsm_tw_xy_ms)
```
We are now left with the same model structure as `dsm_nb_xy_ms`, but by rather a different path. This demonstrates some potential instabilities in the model and the effect of using a different response distribution (and different mean-variance relationship).
...
par(mfrow=c(1,2))
qq.gam(dsm_nb_xy_ms, main="Negative binomial", rep=200)
qq.gam(dsm_tw_xy_ms, main="Tweedie", rep=200)
```

There's not much between these two (all points inside the reference bands). I might be inclined to pick the negative binomial model as the points are slightly closer to the line in the Q-Q plot here.
</div>
...
Starting with negative binomial. To save space I've commented the terms out as I've gone and annotated them with numbers to show the sequence in which terms dropped out of the model.
...
par(mfrow=c(1,2))
qq.gam(dsm_nb_xy_ms_ae, main="Negative binomial", rep=200)
qq.gam(dsm_tw_xy_ms_ae, main="Tweedie", rep=200)
```

This is a close call, neither distribution appears to perform considerably better than the other in Q-Q plot terms. We might prefer the Tweedie model as there is a smaller distance between point and line at larger values.
</div>
...
Reading these matrices can be a little laborious and not much fun. Here is a little bit of code to visualise the concurvity *between terms* in a model by colour coding the matrix.

Again compare these plots for models with different terms.
...
Note that the argument `select=` picks just one term to plot. These are in the order in which the terms occur in the `summary()` output (so you may well need to adjust the above code).
...
par(mfrow=c(2,2))
qq.gam(dsm_nb_xy_ms, main="Negative binomial, s(x,y)", rep=200)
qq.gam(dsm_tw_xy_ms, main="Tweedie, s(x,y)", rep=200)
qq.gam(dsm_nb_x_y_ms, main="Negative binomial, s(x)+s(y)", rep=200)
qq.gam(dsm_tw_x_y_ms, main="Tweedie, s(x)+s(y)", rep=200)
```
