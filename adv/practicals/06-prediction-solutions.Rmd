...
date: Solution 6. Prediction using fitted density surface models
output: 
  pdf_document:
...
<div class="boxout">
We now want to plot the predictions for the 8 models that we have so far (4 negative binomial, 4 Tweedie; 4 spatial only, 4 with environmental covariates; 4 with bivariate smooths, 4 with additive spatial effects).

Duplicating the above code is a bit tiresome, and can be prone to errors, and so we show how we can use the `ldply()` function from the `plyr` package to do the same task many times.
...
We'll need to use the prediction grid and predictor stack again when we calculate uncertainty in the next exercise, so let's save those objects now to save time later.
